...
for xSliceNow = 1:numel(xVect)
    ...
    % Return piecewisePoints to proper spatial coordinates
    if ~(isempty(xPiecewise{iter, 1}))
        if (axisToUse == 1)
            xPiecewise{iter, 1} = xPiecewise{iter, 1}(:, [1, 3, 2]);
        elseif (axisToUse == 2) 
            xPiecewise{iter, 1} = xPiecewise{iter, 1}(:, [3, 1, 2]);
        elseif (axisToUse == 3) 
            xPiecewise{iter, 1} = xPiecewise{iter, 1}(:, [1, 2, 3]);
        else
            error("Invalid value for axisToUse.");
        end
    end
